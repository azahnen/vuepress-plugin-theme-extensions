export const applyMdTable = (md, themeOptions) => {
  const isTableRowHeight = themeOptions.tableRowHeight !== false;
  ...
  if (isTableRowHeight) {
    ...
    md.renderer.rules.td_close = function (tokens, idx, options, env, self) {
      return `</div>${defaultTdCloseRenderer(tokens, idx, options, env, self)}`;
    };
  }
};
